...
--7. Create a query to get all rows in the comments table, showing post title (aliased as 'Post Title'), and all the comment's fields
...
SELECT comments.*, posts.title AS "posts_title", posts.url AS "post_url", comments.body AS "comment_body"
FROM comments INNER JOIN posts ON comments.posts_id = posts.id
WHERE posts.created_at < '2015-01-01';
